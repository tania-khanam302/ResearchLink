...
<section className="p-6">
      ...
      <thead className="bg-slate-50 text-slate-700">
        <tr>
          {["File Name", "Student", "Work", "Type", "Upload Date", "Actions"].map((t, i) => (
            <th key={i} className="sticky top-0 z-30 bg-white py-0 pb-3 px-3 text-left text-sm font-semibold border-b border-slate-200 shadow-sm">{t}</th>
          ))}
        </tr>
      </thead>
        ...
        {currentFiles.map((file) => (
          <tr key={file.id} className="border-t hover:bg-slate-50 transition-colors">
            ...
            <td className="py-2 px-3">
              <div className="flex items-center gap-2">
                <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${file.workType === "thesis" ? "bg-purple-50" : "bg-cyan-50"}`}>
                  {getFileIcon(file.type)}
                </div>

                <span className="font-medium text-xs text-slate-700 truncate " title={file.name}>
                  {file.name}
                </span>
              </div>
            </td>
            ...
            {/* Student */}
            <td className="py-2 px-3">
              <span className="font-medium text-xs text-slate-700 truncate block " title={file.student}>
                {file.student}
              </span>
            </td>
            ...
            {/* Project / Thesis */}
            <td className="py-2 px-3">
              <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide ${file.workType === "thesis" ? "bg-purple-100 text-purple-700" : "bg-cyan-100 text-cyan-700"}`}>
            ...
            {/* File Type */}
            <td className="py-2 px-3">
              <span className="font-semibold text-xs text-slate-600">{file.type}</span>
            </td>
            ...
            <td className="py-2 px-3">
              <div className="flex items-center justify-center gap-1.5">
                ...
                {/* Download */}
                <button onClick={() => handleDownloadFile(file)} className="inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md bg-[#17a2b8] hover:bg-[#138496] text-white text-xs font-medium transition-all" title="Download file">
                  <ArrowDownToLineIcon className="w-3.5 h-3.5" />
                  <span>Download</span>
                </button>

                {/* Delete */}
                <button onClick={() => handleDeleteFile(file)} className="inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md bg-red-50 text-red-600 border border-red-200 hover:bg-red-500 hover:text-white hover:border-red-500 text-xs font-medium transition-all" title="Delete file">
  ...
  {currentFiles.length === 0 && (
    <div className="py-12 flex flex-col items-center justify-center">
      ...
      <h3 className="text-base font-semibold text-slate-600">No files found</h3>
      <p className="text-xs text-slate-400 mt-1">No files match your current filters.</p>
    </div>
  )}
</div>
