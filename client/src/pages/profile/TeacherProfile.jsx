    ...
    <div className="min-h-screen space-y-6 p-2 sm:p-4">
        ...
        <div className="flex flex-col gap-4 px-6 py-6 sm:flex-row sm:items-center sm:px-8">
          <div className="relative h-16 w-16 shrink-0">
            {authUser?.profilePicture ? (
              <img src={authUser.profilePicture} alt="Teacher profile" className="h-16 w-16 rounded-2xl object-cover ring-1 ring-cyan-200" />
            ) : (
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-wider text-cyan-600">Teacher Account</p>
            <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">{authUser?.name || "Teacher Profile"}</h1>
            <p className="mt-1 text-sm text-slate-500">Manage your professional information and account security.</p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="mb-6 flex items-center gap-3 border-b border-slate-100 pb-5">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">Teacher Information</h2>
            <p className="mt-1 text-sm text-slate-500">Keep your professional details up to date.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleProfileSubmit} className="grid gap-5 sm:grid-cols-2">
          {[
            ...
            ["department", "Department", "text", Building2],
            ["expertise", "Expertise", "text", BookOpen],
            ...
            <div key={name}>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={name === "expertise" ? "e.g. AI, Web Development" : `Enter your ${label.toLowerCase()}`}
                  className="input w-full pl-10 placeholder-gray-400 focus:ring-1 focus:ring-[#17a2b8]"
                />
              </div>
              {name === "expertise" && <p className="mt-1 text-xs text-slate-500">Separate multiple areas with commas.</p>}
            </div>
          ))}
            ...
            <div className="flex min-h-[44px] items-center gap-5 px-1 py-2.5">
              {["Male", "Female", "Other"].map((gender) => (
                <label key={gender} className="flex cursor-pointer items-center gap-2 text-sm font-medium text-slate-700">
                  <input type="radio" name="gender" value={gender} checked={profileData.gender === gender} onChange={handleProfileChange} className="h-4 w-4 accent-[#17a2b8]" />
                  {gender}
                </label>
              ))}
            </div>
